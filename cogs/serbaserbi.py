...
class SerbaSerbiGame(commands.Cog, name="🕹️ Serba-Serbi"):
    ...
    # =======================================================================================
    # ---> BAGIAN INI TELAH DIREVISI TOTAL UNTUK PETUNJUK BERTAHAP <---
    # =======================================================================================
    ...
    async def siapakahaku(self, ctx):
        ...
        for i, item in enumerate(questions):
            ...
            for clue_index, clue in enumerate(clues):
                ...
                try:
                    ...
                    # Jalankan listener dengan timeout 10 detik
                    await asyncio.wait_for(listen_for_answer(), timeout=10.0)

                except asyncio.TimeoutError:
                    if clue_index == len(clues) - 1: # Jika ini adalah petunjuk terakhir
                        await ctx.send(f"Waktu habis! Jawaban yang benar adalah **{item['name']}**.")
                    else:
                        continue # Lanjut ke petunjuk berikutnya

            # Setelah semua petunjuk atau ada pemenang
            if winner:
            ...
            # Membersihkan timeout untuk semua user yang kena di ronde ini
        ...
        # Tampilkan leaderboard di akhir game
        if leaderboard:
            ...
            for reaction in cached_msg.reactions:
                if str(reaction.emoji) in ["✅", "❌"]:
                    ...
                    async for user in reaction.users():
                        if not user.bot and user.id not in rewarded_users: self.give_rewards(user, ctx.guild.id); rewarded_users.add(user.id)
    ...
    async def matamata(self, ctx):
        ...
        if ctx.channel.id in self.active_games:
            ...
            self.give_rewards(spy, ctx.guild.id); self.end_game_cleanup(ctx.channel.id)
    ...
    async def tuduh(self, ctx, member: discord.Member):
        ...
        if member.id == spy.id:
            ...
            await ctx.send(f"**Tuduhan Salah!** {member.mention} bukan mata-matanya. **Mata-mata ({spy.mention}) menang!** Lokasinya adalah **{location}**."); self.give_rewards(spy, ctx.guild.id)
        self.end_game_cleanup(ctx.channel.id)
        if ctx.channel.id in self.bot.active_spyfall_games: del self.bot.active_spyfall_games[ctx.channel.id]
